...
const FooterSection = ({ theme, setTheme, isCollapsed, toggleCollapse }: { theme: string, setTheme: Function, isCollapsed: boolean, toggleCollapse: Function }) => (
  <footer className="absolute bottom-0 px-3 py-2">
    <TooltipProvider delayDuration={100}>
      <Tooltip>
        <TooltipTrigger asChild>
          <Button className="text-muted-foreground "variant="ghost" size="icon" onClick={() => setTheme(theme === "light" ? "dark" : "light")}>
  ...
  <Tooltip>
    <TooltipTrigger asChild>
      <Button onClick={(event) => toggleCollapse()} className="p-2 text-muted-foreground" variant="ghost">
        {isCollapsed ? <PanelLeftOpen className="h-[1.2rem] w-[1.2rem]" /> : <PanelLeftClose className="h-[1.2rem] w-[1.2rem]" />}
      </Button>
    </TooltipTrigger>
  ...
  return (
    <div className={`p-4 sidebar flex flex-col h-full border bg-muted ${className} ${isCollapsed ? 'collapsed' : ''}`}>
      <div className=" flex flex-col flex-grow overflow-auto">
        ...
        <FooterSection theme={theme} setTheme={setTheme} isCollapsed={isCollapsed} toggleCollapse={toggleCollapse} />
      </div>
    </div>
    </div>
  );
